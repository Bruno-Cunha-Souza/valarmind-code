...
export function createToolRegistry(_config: ResolvedConfig, _logger: Logger, _fs: FileSystem): ToolRegistry {
  ...
  registry.registerForAgent('search', [
    'read_file', 'glob', 'grep', 'tree_view', 'git_diff',
  ]);

  registry.registerForAgent('research', [
    'read_file', 'glob', 'grep', 'web_search', 'web_fetch',
  ]);

  registry.registerForAgent('code', [
    'read_file', 'write_file', 'edit_file', 'glob', 'grep',
  ]);

  registry.registerForAgent('review', [
    'read_file', 'glob', 'grep', 'git_diff',
  ]);

  registry.registerForAgent('test', [
    'read_file', 'write_file', 'edit_file', 'glob', 'grep', 'bash',
  ]);

  registry.registerForAgent('docs', [
    'read_file', 'write_file', 'edit_file', 'glob', 'web_fetch',
  ]);

  registry.registerForAgent('qa', [
    'read_file', 'glob', 'grep', 'bash', 'git_diff',
  ]);

  registry.registerForAgent('init', [
    'read_file', 'write_file', 'glob', 'grep', 'tree_view',
  ]);
